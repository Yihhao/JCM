...
def compute(N, walist, wc, chi, use_rwa):
...
def main():
    ...
    evals_mat = compute(N, walist, wc, chi, use_rwa)
    fig, ax = plt.subplots(figsize=(12, 6))
    for n in [0, 1, 2, 3]:
        ax.plot(walist / (2 * pi), (evals_mat[:, n] - evals_mat[:, 0]) / (2 * pi), 'b')
